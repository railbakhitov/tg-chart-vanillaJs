...
import { NightTheme, LightTheme } from './theme'
...
export function chart(root, data) {
    ...
    let prevMax
    let themeFlag = 'day'
    let themeOptions = new LightTheme();
    ...
    function changeTheme() {
        if (themeFlag === 'day') {
            themeFlag = 'night'
            themeOptions = new NightTheme()
        } else if (themeFlag === 'night') {
            themeFlag = 'day'
            themeOptions = new LightTheme()
        }
    ...
    function mousemove({ clientX, clientY }) {
        ...
        proxy.mouse = {
            ...
            tooltip: {
                left: clientX - left,
                top: clientY - top,
                backgroundColor: themeOptions.tooltip.backgroundColor,
                color: themeOptions.tooltip.color
            },
        }
    }
            ...
            .forEach((coords, idx) => {
                ...
                for (const [x, y] of coords) {
                    if (isOver(proxy.mouse, x, coords.length, DPI_WIDTH)) {
                        circle(ctx, [x, y], { color, backgroundColor: themeOptions.backgroundColor })
                        break
                    }
                }
            })
